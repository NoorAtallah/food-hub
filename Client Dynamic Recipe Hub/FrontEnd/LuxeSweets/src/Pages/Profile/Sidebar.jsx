...
import { MessageSquare, Star, ShoppingBag, User, ChevronRight, ChevronLeft } from "lucide-react";
...
function Sidebar() {
  ...
  const { data, loading, error } = useFetch('http://localhost:1001/api/auth/getAllUsers');
  ...
  return (
    <>
      ...
      <div 
        className={`fixed inset-y-0 left-0 bg-white shadow-md z-40 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:w-64`}
